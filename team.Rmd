...
```{r}
dat1$political <- factor(dat1$`Political party affiliation`, levels= c('Strong democrat', 'Not str democrat','Ind,near dem','Independent','Ind,near rep','Not str republican','Strong republican', 'Other party', 'Don\'t know', 'No answer' ))
...
```{r}
dat1$labor <- factor(dat1$`Labor force status`, levels= c('Working fulltime', 'Working parttime','Temp not working','unempl, laid off','Retired','School','Keeping house'))
dat1$degree <- factor(dat1$`Rs highest degree`, levels= c('Lt high school', 'High school','Junior college','Bachelor','Graduate','Don\t know'))
ggplot(data = dat1, aes(x=degree, fill=happy)) + geom_bar(position = 'Dodge') + theme(axis.text.x=element_text(angle=90,hjust=1,vjust=0.5)) + facet_wrap(vars(labor)) +xlab("degree") + ylab("Number of respondents") + labs(title="degree obtained by Happiness level, faceted by labor")
```
