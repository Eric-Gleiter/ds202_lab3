...
dat1 <- dat[!is.na(dat$'Marital status'), ]
...
dat1 <- dat1[dat1$`Age of respondent` != 'Don\'t know',]
dat1$age = as.numeric(dat1$`Age of respondent`)
dat1$age = dat1[!is.na(dat1$age),]
...
ggplot(data = dat1, aes(x=politicalParty, y=age)) + geom_boxplot() + theme(axis.text.x=element_text(angle=90,hjust=1,vjust=0.5)) + facet_wrap(vars(sex)) +xlab("Age") + ylab("Number of respondents") + labs(title="Number of Responents vs Maritial Status by Happiness level, faceted by sex")
